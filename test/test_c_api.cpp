...
using namespace simplecs::c_api;
...
TEST(c_api_registry, reg_components_one_component_descriptor_nullptr)
{
    entity_descriptor entity{ 0 };
    std::vector<reg_error> regResult{};
    ASSERT_EXIT((register_components(entity, nullptr, 1, regResult.data(), nullptr), exit(0)),
...
TEST(c_api_registry, reg_components_one_component_reg_error_nullptr)
{
    ...
    std::vector<component_descriptor> componentsEntity{};
    ASSERT_EXIT(
        (register_components(entity, componentsEntity.data(), 1, nullptr, nullptr), exit(0)),
        ::testing::ExitedWithCode(0),
        ".*");
}
...
TEST(c_api_registry, reg_components_ten_already_registered)
{
    ...
    componentDescriptor->typeDescriptor.typeId = 1;
    register_components(entity, componentDescriptor.get(), 1, nullptr, nullptr);
...
TEST(c_api_registry, reg_components_ten_two_already_registered)
{
    entity_descriptor entity{ 0 };
    auto componentDescriptor = std::make_unique<component_descriptor>();
    componentDescriptor->typeDescriptor.typeId = 0;
    register_components(entity, componentDescriptor.get(), 1, nullptr, nullptr);
    ...
    componentsEntity = { component_descriptor{ ++iter }, component_descriptor{ ++iter },
                         ...
                         component_descriptor{ ++iter }, component_descriptor{ ++iter },
                         component_descriptor{ ++iter }, component_descriptor{ ++iter } };
    register_components(entity, componentsEntity.data(), componentsEntity.size(), regResult.data(), nullptr);
...
{
    entity_descriptor entity{ 0 };
    std::vector<type_descriptor> componentsEntity{};
...
TEST(c_api_registry, unreg_components_one_component_descriptor_nullptr)
{
    entity_descriptor entity{ 0 };
    std::vector<unreg_error> unregResult{};
    ASSERT_EXIT((unregister_components(entity, nullptr, 1, unregResult.data(), nullptr), exit(0)),
                ::testing::ExitedWithCode(0),
                ".*");
}
...
TEST(c_api_registry, unreg_components_one_component_reg_error_nullptr)
{
    entity_descriptor entity{ 0 };
    std::vector<component_descriptor> componentsEntity{};
    ASSERT_EXIT((register_components(entity, componentsEntity.data(), 1, nullptr, nullptr), exit(0)),
                ::testing::ExitedWithCode(0),
                ".*");
}
...
//    unregister_components(entity, &component, componentsEntity.size(), unregResult.data(), nullptr);
    FAIL();
...
TEST(c_api_registry, selec_entities)
{
    ...
    std::vector<type_descriptor> selectComponents{ type_descriptor{ sizeof(int), 2 },
                                                   type_descriptor{ sizeof(int), 3 },
                                                   type_descriptor{ sizeof(int), 5 } };
    ...
    EXPECT_EQ(entitySelection.handle, handle{size_t(-1)});
